...
//            }
    ...
    @Path("retrieveListingByOfferId/{offerId}")
    @GET
    @Consumes(MediaType.TEXT_PLAIN)
    @Produces(MediaType.APPLICATION_JSON)
    public Response retrieveListingByOfferId(@QueryParam("username") String username,
            @QueryParam("password") String password,
            @PathParam("offerId") Long offerId) {
        try {
            UserEntity userEntity = userEntitySessionBeanLocal.userLogin(username, password);
            System.out.println("********** ListingResource.retrieveListingByOfferId(): User " + userEntity.getUsername() + " login remotely via web service");

            ListingEntity l = listingEntitySessionBeanLocal.retrieveListingByOfferId(offerId);
            System.out.println("HII" + l.getName());
    ...
    public Response createNewListing(CreateListingReq createListingReq) {
        if (createListingReq.getName()!= null) {
            try {
                ...
                l.setRentalAvailability(createListingReq.isRentalAvailability());
                l.setForSaleAvailability(createListingReq.isForSaleAvailability());
